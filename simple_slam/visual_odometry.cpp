...
VisualOdometry::VisualOdometry(const std::string &data_dir) : data_loader_(data_dir)
{
...
std::vector<cv::Point2f> VisualOdometry::extract_features(const cv::Mat &img)
{
    ...
    cv::goodFeaturesToTrack(img, corners, max_corners, quality_level, min_distance);
    return corners;
}

Features VisualOdometry::get_matched_features(const cv::Mat &img1, const cv::Mat &img2)
{
...
{
    cv::Point3d prev_point = {pred_poses_[index-1].at<double>(0,3), pred_poses_[index-1].at<double>(1,3), pred_poses_[index-1].at<double>(2,3)};
    cv::Point3d curr_point = {pred_poses_[index].at<double>(0,3), pred_poses_[index].at<double>(1,3), pred_poses_[index].at<double>(2,3)};
...
void VisualOdometry::run_vo_pipeline()
{
    ...
    while (img_pair.has_value()) {
        ...
        E = cv::findEssentialMat(features.second, features.first, cam_matrix_, cv::RANSAC, 0.999, 1.0);

        // extract pose
        int inlier_cnt = cv::recoverPose(E, features.second, features.first, cam_matrix_, R, t);
